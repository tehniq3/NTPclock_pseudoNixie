...
// 1. Adaugata functie 'verificaWiFi()' in loop pentru a forta reconectarea.
//    Funcția standard WiFi.setAutoReconnect(true) pe ESP32 eșuează adesea 
//    după o cădere prelungită a routerului. Soluția este să verificăm manual 
//    starea WiFi și să apelăm WiFi.reconnect() dacă legătura s-a pierdut.
// 2. Adăugată variabilă globală 'lastReconnectAttempt' pentru a nu bloca 
//    procesorul cu încercări continue de reconectare (se încearcă o dată la 10 sec).
// 3. Adăugat feedback vizual: Dacă WiFi este căzut, LED-ul de secunde clipește 
//    rapid (de 4 ori mai repede) pentru a semnala problema.
// 4. Adăugat Serial.begin(115200) în setup pentru a putea vedea în Monitorul 
//    Serial mesajele de debug (ex: "WiFi pierdut!").
// =============================================================================
...
//
// FACILITATI:
// - Afisaj ceas Nixie simulat pe 4 ecrane
// - Sincronizare oră automată prin NTP (Internet)
// - WiFiManager pentru configurare ușoară rețea (fără codare SSID/Parolă)
// - Auto-reconectare WiFi FORȚATĂ în caz de deconectare (MODIFICARE)
// - Stingere automată a "zecilor" de ore dacă cifra este 0 (ex: 8:00)
// - Imagini comprimate RLE pentru economie de memorie
...
#include <WiFiManager.h> // <--- Bibliotecă necesară pentru configurare WiFi
...
unsigned long lastReconnectAttempt = 0; // <-- MODIFICARE: Variabilă pentru a controla intervalul de reconectare
...
// <-- MODIFICARE: Funcție dedicată pentru verificarea și forțarea reconectării WiFi
void verificaWiFi() {
  if (WiFi.status() == WL_CONNECTED) {
    return; // Dacă e conectat, nu facem nimic
  }

  // Dacă nu e conectat, încercăm reconectarea la fiecare 10 secunde
  ...
  if (acum - lastReconnectAttempt > 10000) {
    ...
    WiFi.disconnect(); // Curățăm starea veche
    WiFi.reconnect();  // Forțăm reconectarea folosind credențialele salvate de WiFiManager
  }
}


void setup() {
  Serial.begin(115200); // <-- MODIFICARE: Pornire Serial pentru debug
  ...
  WiFiManager wm;
  // Dacă nu sunt setate credențiale, se pornește portalul de config la IP 192.168.4.1
  // Numele rețelei create de ceas: "NixieClock_Setup"
  bool res = wm.autoConnect("NixieClock_Setup");

  if(!res) {
    // Dacă configurarea eșuează, poți adăuga aici logică de restart sau așteptare
    Serial.println("Configurare WiFi esuata!"); // <-- MODIFICARE: Mesaj debug
  } else {
    // Activăm reconectarea automată în cazul căderii semnalului (de siguranță)
    WiFi.setAutoReconnect(true);
    WiFi.persistent(true);
    Serial.println("WiFi conectat!"); // <-- MODIFICARE: Mesaj debug
  }
  // ---------------------------------------
...
void loop() {
  // <-- MODIFICARE: Verificăm starea WiFi la fiecare iterație
  verificaWiFi();

  // <-- MODIFICARE: Dacă nu avem WiFi, arătăm un semnal vizual și oprim actualizarea ceasului
  if (WiFi.status() != WL_CONNECTED) {
    // Clipim LED-ul de secunde mult mai repede (250ms) ca semn de avertizare că nu avem rețea
    digitalWrite(LED_SECUNDE, !digitalRead(LED_SECUNDE)); 
    delay(250);
    return; // Sărim peste restul loop-ului până ne reconectăm
  }
  ...
  if (!getLocalTime(&timeinfo)) {
    // Dacă nu avem timp (NTP nu a răspuns încă), așteptăm puțin
    delay(1000);
    return; 
  }
  ...
  // Dacă ora este mai mică de 10 (ex: 09), facem ca primul tub să fie stins (10)
  if (h1 == 0) {
    h1 = 10; // 10 este codul pentru imaginea "blank"
  }
  // ---------------------------------------------------------
