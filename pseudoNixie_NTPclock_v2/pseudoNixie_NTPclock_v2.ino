...
// - Auto-reconectare WiFi în caz de deconectare
...
void setup() {
  ...
  if(!res) {
    ...
    // Momentan continuă să încerce în bucla loop
  } else {
    // Activăm reconectarea automată în cazul căderii semnalului
...
void loop() {
  ...
  if (!getLocalTime(&timeinfo)) {
    // Dacă nu avem timp (poate WiFi e down), nu facem nimic un moment
    // WiFiAutoReconnect va încerca în fundal să restabilească legătura
    delay(1000);
    return; 
  }
